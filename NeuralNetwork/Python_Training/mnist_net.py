...
def main():
	...
	for j in range(2): # train and test	
		for i in range(3): # 1 to 3, in order: bolt,nut,screw
			if j == 0:
				read_folder = dataset_dir + '/Training/' + str(i+1) + '/'
			if j == 1:
				read_folder = dataset_dir + '/Testing/' + str(i+1) + '/'
	...
	model = Sequential([
		Flatten(input_shape=dims),		# reshape 32x24 to 100, layer 0
		Dense(80, activation='relu',use_bias=False),
	...
	print("Lets now test for an ammount of data:")
	for image in range(5):
		x = test_images[image]
		img_filename = f"img_{image}_Value_{test_labels[image]}.txt" 
		open(img_filename, 'w').close() # clear file
		file = open(img_filename,"a") 
		file.write('{')
		for i in range(dims[1]):
		...
		print(f"Inputted: {test_labels[image]}")
